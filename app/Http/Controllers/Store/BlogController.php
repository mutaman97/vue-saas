...
class BlogController extends Controller
{
    ...
    {
        $info=Term::query()->where('type','blog')->where('status',1)->where('slug',$slug)->with('excerpt','description','preview','thum_image')->first();
        abort_if(empty($info),404);
        $recent_posts=Term::query()->where('type','blog')->where('status',1)->with('preview','thum_image')->latest()->take(3)->get();

        // $data['relatedPosts'] = $this->blogModel->getRelatedPosts($data['post']->category_id, $data['post']->id);
        $related_posts=Term::query()->where('type','blog')->where('status',1)->with('preview','thum_image')->oldest()->take(3)->get();
        ...
        return  view(baseview('blog/show'),compact('info','recent_posts','related_posts'));
    }
}

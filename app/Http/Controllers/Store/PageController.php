...
class PageController extends Controller
{
    public function home()
    {
        ...
        $store_title=$home_data->seo->site_title;

        // $indexBannersArray = $this->commonModel->getIndexBannersArray();
        ...
        $indexBannersArray=Category::where('type','short_banner')->where('status',1)->get();
        ...
        // $categories=$orderby == 'ASC' ? $categories : $categories->get();
        // $categories= $with == '' ? $categories : $categories->with($with);

        // $indexBannersArray = $categories;

        return view(baseview('index'),compact('page_data','store_title','indexBannersArray'));
    }
    ...
    {
        // disabled reviews count -- by mutaman
        // $info=Term::query()->where([['type','product'],['status',1]])->with('tags','category','brands','excerpt','description','preview','thum_image','medias','optionwithcategories','price','seo','discount')->withCount('reviews')->where('slug',$slug)->withTotalVisitCount()->first();
        $info=Term::query()->where([['type','product'],['status',1]])->with('tags','category','brands','excerpt','description','preview','thum_image','medias','optionwithcategories','price','seo','discount')->where('slug',$slug)->withTotalVisitCount()->first();
        abort_if(empty($info),404);
        $seo=json_decode($info->seo->value ?? '');

        $medias=json_decode($info->medias->value ?? '');

        // Added By Mutaman For Review Button in product details page
        $productId=$info->id;
        $hasPurchased = Auth::check() ? Auth::user()->orders()->whereHas('orderItems', function ($query) use ($productId) {$query->where('term_id', $productId);})->exists() : false;
        ...
        $info->visit()->yearlyIntervals();

        return view(baseview('details'),compact('info','preview','galleries','home_data','hasPurchased'));
    }

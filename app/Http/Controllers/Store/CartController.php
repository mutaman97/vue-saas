...
class CartController extends Controller
{
    ...
    public function addtowishlist(Request $request){
        ...
        if (auth()->check()) {
            ...
            $isStored = DB::table('shoppingcart')
    ...
    {
        if (Cart::count() == 0) {
             $errors['errors']['error']=__('Please add some product in your cart');
        ...
        if ($coupon == null) {
             $errors['errors']['error']= __('Sorry this coupon code is unavailable');
             return response()->json($errors,401);
        }
        ...
        if ($coupon->is_conditional == 1) {
            ...
            if ($total_amount < $coupon->min_amount) {
                $errors['errors']['error']=__('The minumum order amount, for this coupon is ').number_format($coupon->min_amount,2);
                return response()->json($errors,401);
           }
        ...
        return response()->json(__('Coupon Code Applied Successfully'));
    }


    // added by mutaman for remove coupon code
    ...
    public function express(Request $request)
    {
       ...
       if ($request->qty == 0) {
           $errors['errors']['error']=__('Oops, Minimum cart quantity is 1 product');
           return response()->json($errors,401);
       }
        ...
        if ($info->is_variation == 1) {
                ...
                if ($final_price == 0) {
                    $errors['errors']['error']=__('Please Select any Option, variation field is required');
                    return response()->json($errors,401);
                }

                if ($stock_status == false) {
                    $errors['errors']['error']=__('Out Of Stock');
                    return response()->json($errors,401);
                }

                 if ($stock_limit == false) {
                    $errors['errors']['error']=__('Oops, Maximum stock limit exceeded');
                    return response()->json($errors,401);
                }
       ...
       if ($request->qty == 0) {
        ...
        if ($info->is_variation == 1) {
            ...
            if ($price->stock_status == 0) {
                $errors['errors']['error']=__('Opps, Maximum stock limit exceeded');
                return response()->json($errors,401);
        ...
        // TODO
        // $productcartdata['cart_discount']= Cart::discount();
        // logger($productcartdata['cart_discount']);

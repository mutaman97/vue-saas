...
class SitesettingsController extends Controller
{
    ...
    public function show($slug)
    {
       ...
       if ($slug == 'general') {
           ...
           $theme_color=Option::where('key','theme_color')->first();
           ...
           return view('seller.settings.general',compact('languages','store_sender_email','theme_color','invoice_data','timezone','default_language','weight_type','currency_info','average_times','order_method','order_settings','whatsapp_no','whatsapp_settings'));
       }
    ...
    public function update(Request $request, $id)
    {
        if ($id == 'general') {
           $validated = $request->validate([
                ...
                'theme_color' => 'required|starts_with:#|string:3,6',
                ...
                'store_legal_description' => 'required|max:400',
           ...
           if (empty($theme_color)) {
              $theme_color=new Option;
              $theme_color->key='theme_color';
              $theme_color->autoload=1;
           }
           $theme_color->value=json_encode($request->theme_color);
           $theme_color->save();

           TenantCacheClear('theme_color');
           ...
           $invoice_info['store_legal_description']=$request->store_legal_description;
           ...
           return response()->json('General Settings Updated Successfuly');
        }
    ...
    public function setStore(Request $request)
    {
        $validated = $request->validate([
            ...
            'theme_color' => 'required|starts_with:#|string:3,6',
            'whatsapp_no' => ['required', 'string', 'starts_with:249', 'size:12'],
            // 'store_sender_email' => 'required|email|max:50',
            // 'logo' => 'mimes:png|max:200',
            'default_language' => 'required|max:50',
       ]);
        ...
        $option = Option::firstOrNew(['key' => 'seo']);
        $data = json_decode($option->value ?? '{}', true);
        $data['title'] = $seo_data['title'];
        $data['description'] = $seo_data['description'];
        $option->value = json_encode($data);
        $option->save();

        TenantCacheClear('seo');
        ...
        $default_language->value=$request->default_language ?? 'ar';
        $default_language->save();

        TenantCacheClear('default_language');
        ...
        //    $whatsapp_no->autoload=1;
        // }
        // $whatsapp_no->value=$request->whatsapp_number;
        // $whatsapp_no->save();
        ...
        $whatsapp_settings_value = json_decode($whatsapp_settings->value, true);
        $whatsapp_settings_value['whatsapp_no'] = $request->whatsapp_no;
        $whatsapp_settings->value = json_encode($whatsapp_settings_value);
        $whatsapp_settings->save();
        ...
        //  return response()->json('Store Settings Updated Successfuly');

...
class SettingsController extends Controller
{
    ...
    public function custom_css_js_update(Request $request)
    {
        ...
        if (tenant('custom_css_js') != 'on') {
            $errors['errors']['error']=__('Sorry... This option is not supported in your plan, please upgrade');
    ...
    {
        if (tenant('pwa') != 'on') {
            $errors['errors']['error']=__('Sorry... PWA Modules is not supported in your plan, please upgrade');
            return response()->json($errors,401);
        }
...
\File::put('uploads/'.$user_id.'/manifest.json',$mainfest);
        return response()->json(__('PWA Sucessfully Updated'));
    }
}

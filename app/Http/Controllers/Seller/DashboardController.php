...
class DashboardController extends Controller
{
   ...
   public function cacheClear(){
     ...
     return response()->json(__('Store cache cleared'));
   }
    ...
    public function subscriptionStatus()
    {
         ...
         foreach($features as $key => $value){
            if ($key=='storage_limit') {
                if ((int)tenant($key) == -1) {
                    $data[ucwords(str_replace('_',' ',$key))]='Unlimited';
                }
                else{
                    $data[ucwords(str_replace('_',' ',$key))]=number_format(tenant($key),2).' - '.number_format(Media::sum('size'),2).'MB';
                }
            ...
            elseif($key=='post_limit'){
                if ((int)tenant($key) == -1) {
                    ...
                    $data[ucwords(str_replace('_',' ',$key))]=tenant($key).' - '.Term::count()+Category::count();
                }
            }
            ...
            elseif($key=='staff_limit'){
                if ((int)tenant($key) == -1) {
                    ...
                    $data[ucwords(str_replace('_',' ',$key))]=tenant($key).' - '.$admins+$riders;
                }
            }
                ...
                $data[ucwords(str_replace('_',' ',$key))]=tenant($key) != null ? tenant($key) : '';
            }
    ...
    public function staticData()
    {
        ...
        $data['out_of_stock_products'] = Term::with('preview')->whereHas('prices', function($query){ $query->where('stock_status', 0);})->with('reviews')->withCount('reviews')->orderBy('reviews_count','DESC')->take(10)->get();
        // end added

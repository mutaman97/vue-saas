...
class PosController extends Controller
{
    public function index()
    {
        ...
        $currency_name = get_option('currency_data',true)->currency_name;
        return view('seller.pos.pos',compact('categories','getways','tables','locations','order_status','currency_name'));
    }
        ...
        elseif(!empty($request->barcode)){
            ...
            if (!empty($info)) {
              if ($info->is_variation == 1) {
                ...
                $errors['errors']['error']=__('Opps this product is not available');
                return response()->json($errors,401);
            }
        }
    ...
    public function makeorder(Request $request)
    {
        ...
        if (!empty($request->coupon)) {
           ...
           $coupon=Coupon::where('code',$request->coupon)
                    ...
                    ->first();
            if ($coupon == null) {
                 $errors['errors']['error']=__('Opps this coupon code is not available');
                 return response()->json($errors,401);
            }
        ...
        Cart::destroy();
           ...
           DB::commit();
            ...
            DB::rollback();
            return $th;
            $errors['errors']['error']= __('Opps something went wrong');
            return response()->json($errors,401);
        }

        return response()->json(__('Order Placed Successfully'));
    }
}

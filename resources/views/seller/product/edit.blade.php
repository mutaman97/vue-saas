...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>__('Edit Product'),'prev'=> url('seller/product')])
@endsection
...
<div class="tab-pane fade show active" id="general_info" role="tabpanel" aria-labelledby="home-tab4">
    <div class="from-group row mb-2">
        ...
        <div class="col-lg-12">
            <input type="text" name="name" required="" class="form-control" placeholder="{{ __('Enter Product Name') }}" value="{{ $info->title }}">
        </div>
    </div>
    <div class="from-group row mb-2">
        <label for="" class="col-lg-12">{{ __('Slug') }} </label>
        <div class="col-lg-12">
            <input type="text" name="slug" required="" class="form-control" placeholder="{{ __('Enter Product Slug') }}" value="{{ $info->slug }}">
        </div>
    </div>
    <div class="from-group row mb-2">
    ...
    <div class="from-group row mb-2">
        ...
        <div class="col-lg-12">
         <select name="categories[]"   class="form-control selectric">
           ...
           <option value="{{ $row->id }}" @if(in_array($row->id,$selected_categories)) selected @endif>{{ __($row->name) }}</option>
           @endforeach
        </select>
      </div>
    </div>

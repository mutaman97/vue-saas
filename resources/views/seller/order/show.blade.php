				...
				<div class="card-footer">
					<div class="text-right">
						<form method="POST" action="{{ route('seller.order.update',$info->id) }}" accept-charset="UTF-8" class="d-inline ajaxform">
							...
							<div class="col-sm-4">
							<div class="form-group text-left">
								<label>{{ __('Select Payment Status') }}</label>
								<select class="form-control selectric" name="payment_status" required="">
									<option value=""><b>{{ __('Select Payment Status') }}</b></option>
									<option value="1" @if($info->payment_status=='1') selected="" @endif>{{ __('Completed') }}</option>
									<option value="2" @if($info->payment_status=='2') selected="" @endif>{{ __('Pending') }}</option>
									<option value="0" @if($info->payment_status=='0') selected="" @endif>{{ __('Canceled') }}</option>
									<option value="3" @if($info->payment_status=='3') selected="" @endif>{{ __('Incomplete') }}</option>
								</select>
							</div>
						</div>
							<div class="col-sm-4">
			...
			<div class="card-grouping">
				...
				<div class="card card-primary">
					...
					<div class="card-body">
						...
						<p>{{ __('Payment Method') }}  <span class="badge  badge-success  float-right">{{ $info->getway->name ?? '' }} </span></p>
						<p>{{ __('Transaction ID') }} <span class="float-right">{{ $info->transaction_id ?? '' }}</span></p>
						@else
						<p class ="text-danger">{{ __('Incomplete Payment') }}</p>
						@endif
					</div>
				</div>

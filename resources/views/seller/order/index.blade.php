        ...
        <div class="col-sm-12">
            <ul class="nav nav-pills">
                <li class="nav-item px-1">
                    <a class="nav-link  {{ $request_status == null ? 'active' : '' }} " href="{{ route('seller.order.index') }}">{{ __('All') }}</a>
                </li>
                @foreach($status as $row)
                    <li class="nav-item px-1">
                        <a class="nav-link  {{ $request_status == $row->id ? 'active' : '' }}" href="{{ url('seller/order?status='.$row->id) }}">{{ __($row->name) }}<span class="badge badge-secondary">{{$row->orderstatus_count}}</span></a>
                    </li>
                @endforeach
            </ul>
        </div>
    ...
    <div class="card-header">
        ...
        <form class="card-header-form">
            <div class="input-group">
                <input type="text" name="src" value="{{ $request->src ?? '' }}" class="form-control" required=""  placeholder="{{ __('Order Id') }}" />
                ...
                <table class="table table-hover table-nowrap card-table text-center">
                    ...
                    <tbody class="list font-size-base rowlink" data-link="row">
                        ...
                        <tr>
                            ...
                            <td>{{ __($row->order_method) }}</td>

                            <td>{{ $row->orderitems_count }}</td>
                            <td>
                                <a target="_blank" href="{{ route('seller.order.print',$row->id) }}" class="btn btn-primary">{{ __('Print') }}</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>

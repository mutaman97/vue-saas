...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>__('POS')],['prev'=> 'seller/dashboard'])
@endsection
...
<section>
  <div class="pos-main-area">
    <div class="container-fluid">
      <div class="row">
        <div class="col-lg-8 pos-right-bar p-y-3 border-0">
              ...
              <div class="pos-search-bar">
                ...
                <div class="search-input">
                  <form class="search_form" method="post" action="{{ route('seller.pos.search') }}">
                   @csrf
                  <input type="text" name="src" required="" placeholder="{{ __('Search Product') }}" id="product_search">
                  </form>
                </div>
              </div>
            </div>
            <div class="product-categories-lists">
              <nav>
                <ul class="nav nav-pills " role="tablist">
                  ...
                  <li class="nav-item category_item" data-id="{{ $row->id }}"><a class="category-item" href="#" data-id="{{ $row->id }}">{{ __($row->name) }}</a></li>
                  @endforeach
                </ul>
              </nav>
            </div>
            <div class="pos-product-main-content p-y-3">
          ...
          <div class="pos-right-area">
            <div class="customer-header-input">
              ...
              <div class="customer-input">
                <input type="text" id="customer_email" name="customer_email" placeholder="{{ __('Select Customer') }}">
              </div>
            </div>
            @if(tenant('customer_modules') == 'on')
            <div class="add-new-customer-btn p-3">
          ...
          <div class="form-group mt-3">
            <div class="barcode-input-area">
              <div class="input-group">
                <input type="text" class="barcode" placeholder="{{ __('Enter product barcode') }}">
              </div>
            ...
            <div class="table-responsive">
              <table class="table table-striped">
                ...
                <tr>
                  ...
                  <th class="payable-right subtotal" >{{ Cart::subtotal() }}&nbsp;{{ __($currency_name) }}</th>
                </tr>
                <tr>
                  <th><strong>{{ __('Tax') }}</strong></th>
                  <th class="payable-right tax">{{ Cart::tax() }}&nbsp;{{ __($currency_name) }}</th>
                </tr>
                <tr>
                  <th><strong>{{ __('Total(Payable)') }}</strong></th>
                  <th class="payable-right total">{{ Cart::total() }}&nbsp;{{ __($currency_name) }}</th>
                </tr>
              </tbody></table>
            </div>
          </div>
          <div class="pos-button py-4">
...
@push('script')
    <script>
        var cartAdded = "{{ __('Added To Cart Successfully') }}";

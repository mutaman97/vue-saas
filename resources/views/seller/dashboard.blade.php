...
<div class="row">
  <div class="col-lg-8 col-md-12 col-12 col-sm-12">
    ...
    {{-- added by mutaman for out of stock products --}}
    <div class="card card-primary">
        <div class="card-header">
          <h4>{{ __('Out Of Stock Products') }}</h4>
        </div>
        <div dir="ltr" class="card-body">
          <div class="owl-carousel owl-theme products-carousel out_of_stock_products">
  ...
  <div class="col-lg-4">
    ...
    <div class="card card-primary">
      <div class="card-header">
        <h4>{{ __('Your Store QR Code') }}</h4>
      </div>
      <div class="card-body">
        <div class="qrcode-area">
          <img id="qrcode_img" src="data:image/png;base64,{{ DNS2D::getBarcodePNG(url('/'), 'QRCODE',15,15) }}" alt="QR Code">
        </div>
    ...
    <div class="card gradient-bottom card-primary">
     ...
     <div class="card card-primary">
      <div class="card-header">
        ...
        <div class="card-header-action ">
          <span>{{ __('Expire') }}: {{ \Carbon\Carbon::parse(tenant('will_expire'))->diffForHumans() }}</span>
...
@if(tenant('push_notification') == 'on' && env('FMC_SERVER_API_KEY') != null)
  <div class="notification-button-area notification_button">
    <button id="btn-nft-enable" class="btn btn-primary btn-xs btn-flat notification_button"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M20 17h2v2H2v-2h2v-7a8 8 0 1 1 16 0v7zM9 21h6v2H9v-2z"/></svg></button>
  </div>
@endif

<script>
    var productDetailsText = "{{ __('View') }}";
    var enabledText = "{{ __('Active') }}";
    var disabledText = "{{ __('Inactive') }}";
    var unlimitedText = "{{ __('Unlimited') }}";

    var subDomain = "{{ __('Sub Domain') }}";
    var customDomain = "{{ __('Custom Domain') }}";
    var barCode = "{{ __('Barcode') }}";
    var qrCode = "{{ __('Qr Code') }}";
    var customerModules = "{{ __('Customer Modules') }}";
    var pos = "{{ __('Pos') }}";
    var pushNotification = "{{ __('Push Notification') }}";
    var imageOptimization = "{{ __('Image Optimization') }}";
    var customCss_Js = "{{ __('Custom Css Js') }}";
    var pwa = "{{ __('Pwa') }}";
    var storageLimit = "{{ __('Storage Limit') }}";
    var postLimit = "{{ __('Post Limit') }}";
    var staffLimit = "{{ __('Staff Limit') }}";
    var sdgText = "{{ __('SDG') }}";
...
<script src="{{ asset('admin/assets/js/chart.min.js') }}?v=10009801"></script>
<script src="{{ asset('admin/assets/js/jquery.sparkline.min.js') }}"></script>
<script src="{{ asset('admin/assets/js/seller.js') }}?v=00900"></script>
@endpush

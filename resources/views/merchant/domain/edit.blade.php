...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>__('Edit Store'),'prev'=>route('merchant.domain.list')])
@endsection
                ...
                <form method="POST" action="{{ route('merchant.domain.update',$info->id) }}" class="ajaxform">
                    ...
                    @foreach($arr ?? []  as $key => $row)
                        @if($key == 'storage_limit')
                            <div class="form-group row mb-4">
                                <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __(ucwords(str_replace('_', ' ',$key))) }}</label>
                                <div class="input-group col-sm-12 col-md-7">
                                    {{-- <div class="input-group-prepend">
                                    <span class="input-group-text">$</span>
                                    </div> --}}
                                    <input type="text" disabled="" class="form-control"@if($info->$key == -1)value="{{ __('Unlimited') }}"@else value="{{ $info->$key ?? '' }}"@endif>

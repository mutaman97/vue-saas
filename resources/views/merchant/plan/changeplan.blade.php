    ...
    @foreach ($plans ?? [] as $plan)
       ...
       @endphp
        <div class="col-12 col-md-4 col-lg-4">
            <div class="pricing {{ $plan->is_featured ? 'pricing-highlight' : 'pricing-highlight' }}">
                <div class="pricing-title">
                    {{ __($plan->name) }}
                </div>
                <div class="pricing-padding">
                    <div class="pricing-price">
                        <div>{{ $plan->price }}&nbsp;{{ __($symbol) }}</div>
                        <div>
                            @if ($plan->duration == 7)
                                {{ __('Per Week') }}
                ...
                <div class="pricing-cta">
                    <a href="{{ url('/partner/plancharge/'.$info->id.'/'.$plan->id) }}">{{ __('Subscribe') }} <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
        </div>
    @endforeach
